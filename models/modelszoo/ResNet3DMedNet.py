...
class TranspConvNet(nn.Module):
    ...
    def __init__(self, n_channels, n_classes):
        ...
        transp_conv_1 = nn.ConvTranspose3d(n_channels, transp_channels, kernel_size=2, stride=2)
        ...
        self.conv_final = conv1x1x1(conv_channels, n_classes, stride=1)
...
class ResNetMed3D(BaseModel):
    ...
    def __init__(self, n_channels=3, n_classes=10,
        ...
        self.segm = TranspConvNet(n_channels=512 * block.expansion, n_classes=n_classes)
    ...
    def test(self):
        a = torch.rand(1, self.n_channels, 16, 16, 16)
        y = self.forward(a)
        target = torch.rand(1, self.n_classes, 16, 16, 16)
        assert a.shape == y.shape


def generate_resnet3d(n_channels=1, n_classes=1, model_depth=34, **kwargs):
    ...
    if model_depth == 10:
        model = ResNetMed3D(n_channels=n_channels, n_classes=n_classes, block=BasicBlock,
        ...
        model = ResNetMed3D(n_channels=n_channels, n_classes=n_classes, block=Bottleneck,
                            layers=res_net_dict[model_depth], block_inplanes=in_planes, **kwargs)
