...
class ConvolutionBlock(nn.Module):
    ...
    def __init__(self, in_filters, out_filters, kernel_size=3, batchnorm=True, last_active=F.relu):
        ...
        self.c1 = nn.Conv3d(in_filters, out_filters, kernel_size, padding=1)
        self.b1 = nn.BatchNorm3d(out_filters)
        self.c2 = nn.Conv3d(out_filters, out_filters, kernel_size, padding=1)
        self.b2 = nn.BatchNorm3d(out_filters)
...
class ContractiveBlock(nn.Module):
    ...
    def __init__(self, in_filters, out_filters, conv_kern=3, pool_kern=2, dropout=0.5, batchnorm=True):
        ...
        self.c1 = ConvolutionBlock(in_filters=in_filters, out_filters=out_filters, kernel_size=conv_kern,
                                   batchnorm=batchnorm)
        self.p1 = nn.MaxPool3d(kernel_size=pool_kern, ceil_mode=True)
        self.d1 = nn.Dropout3d(dropout)
...
class ExpansiveBlock(nn.Module):
    ...
    def __init__(self, in_filters1, in_filters2, out_filters, tr_kern=3, conv_kern=3, stride=2, dropout=0.5):
        ...
        self.t1 = nn.ConvTranspose3d(in_filters1, out_filters, tr_kern, stride=2, padding=1, output_padding=1)
...
class Unet_dict(nn.Module):
    ...
    def __init__(self, n_labels, n_filters=32, p_dropout=0.5, batchnorm=True):
        ...
        self.output = nn.Conv3d(filt_pair[1], n_labels, kernel_size=1)
...
#     """Convolution Block using 2 Conv3d
#     Args:
...
#
#         self.conv1 = nn.Conv3d(self.in_channels, self.out_channels, self.kernal_size,  padding=1, bias=True)
#         self.conv2 = nn.Conv3d(self.out_channels, self.out_channels, self.kernal_size, padding=1, bias=True)
#
#         self.b1 = nn.BatchNorm3d(out_channels)
...
#
#         self.conv_last = nn.Conv3d(n1, n_class, kernel_size=1, stride=1, padding=0)
#       #  self.active = torch.nn.Sigmoid()
#
#
#
